...
import { ArrowRight, Wrench, TrendingUp, Phone, ExternalLink } from "lucide-react"
  ...
  return (
    <>
      ...
      <main className="flex min-h-screen flex-col items-center">
        ...
        {/* ヒーローセクション */}
        <section className="w-full bg-gradient-to-r from-emerald-50 to-teal-50 py-24 md:py-36">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-start">
                <div className="flex flex-col items-center text-center space-y-3">
                  <div className="relative w-48 h-48 rounded-full overflow-hidden bg-gray-200">
                    <Image src="/placeholder-user.jpg" alt="代表社員 大野 拓" fill className="object-cover" />
                  </div>
                  <div>
                ...
                <div className="md:col-span-2 space-y-6">
                  <div>
                    ...
                    <ul className="space-y-3">
                      {[
                        { period: "2000〜2010", content: "人材派遣会社にてコールセンター立ち上げ（100名規模）・部門マネジメント（月商1,000〜5,000万円規模）" },
                        { period: "2010〜2017", content: "通信関連企業にて営業組織マネジメント（50名規模）・支店長として入札案件営業・採用・現場運営を統括" },
                        { period: "2017〜2018", content: "コールセンター運営企業にて営業・業務改善" },
                        { period: "2018〜2024", content: "海外発SaaS企業の日本法人立ち上げに参画。サービスのローカライズ・営業・ブランディング・バックオフィス構築まで一貫して担当" },
                        { period: "2025", content: "プログラミングスクール卒業（エンジニアリング習得）" },
                        { period: "2025〜現在", content: "node-bee合同会社 設立・代表" },
                      ].map((item, index) => (
        ...
        <section className="w-full py-16 bg-white" id="services">
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-lg text-gray-600 max-w-3xl mx-auto">現場経験をもとに、今すぐ動ける施策を一緒に考えます</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
              {[
                { title: "AI・SaaSツール導入支援", description: "中小企業の業務に合ったツールを選定し、実際の設定・使い方まで一緒にハンズオンでサポート。freee、Notion、Slack、kintoneなど。", icon: <Wrench className="h-12 w-12 text-emerald-500" /> },
                { title: "業務改善コンサルティング", description: "現状の業務フローを整理し、どこをどう改善すればいいかをわかりやすく提案。難しいITの話は抜きに、すぐ動ける施策を提示。", icon: <TrendingUp className="h-12 w-12 text-emerald-500" /> },
                { title: "コンタクトセンター・DX支援", description: "コールセンターの立ち上げ・運用・DX化を一気通貫でサポート。20年以上の現場経験をもとに、システム導入・業務設計・運用改善まで対応。", icon: <Phone className="h-12 w-12 text-emerald-500" /> },
              ].map((item, index) => (
                <div key={index} className="flex flex-col items-center text-center p-6 bg-gray-50 rounded-lg hover:shadow-md transition-shadow">
          ...
          <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="max-w-3xl mx-auto">
              <Accordion type="single" collapsible className="w-full">
                {[
                  { question: "AI導入に必要な専門知識がなくても大丈夫ですか？", answer: "はい、まったく問題ありません。AI/DXの専門知識がない方でも安心して利用できるよう設計されています。ビジネス課題をお聞かせいただければ、最適なAI活用方法をご提案し、導入から運用までサポートします。" },
                  { question: "どのような業種に対応していますか？", answer: "製造業、小売業、サービス業、士業（税理士、弁護士など）など、幅広い業種に対応しています。業種ごとの特性や課題を理解した上で、最適なAI/DX活用方法をご提案します。" },
                  { question: "自社に合うAIツールがわからないのですが、選定もサポートしてもらえますか？", answer: "もちろんです。現在市場にある多数のAIツールの中から、御社の課題解決に最適なものを選定し、導入方法までアドバイスします。複数のツールを組み合わせた最適なソリューションもご提案可能です。" },
                  { question: "導入後のサポート体制はどうなっていますか？", answer: "定期的なオンライン相談を通じて、導入後の運用状況の確認や新たな課題解決のアドバイス、最新AI技術の情報提供など、継続的にサポートします。まずは無料相談でお気軽にご相談ください。" },
                ].map((faq, index) => (
          ...
          <div className="container mx-auto px-4 md:px-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
              <div>
                ...
                <p className="text-gray-400 text-sm">中小企業・小規模事業者の業務を、AIとSaaSでシンプルにする</p>
              </div>
              <div>
                <h3 className="text-lg font-bold mb-3">プロダクト</h3>
                <ul className="space-y-2">
                  <li>
                    <a href="https://worktrim.vercel.app" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-emerald-400 text-sm flex items-center gap-1">
